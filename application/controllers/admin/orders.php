    ...
    */
    public function lists () {
     ...
     //  	if($month == 12) {
		...
		$total_num = $this->base->get_data('pay_order')->num_rows();
		...
		$this->gpagination->target(site_url('admin/orders/lists'));

		$this->_data['pagination'] = $this->gpagination->getOutput();
		$this->_data['lists'] = $this->base->get_data('pay_order', array(), '*', $limit, $offset)->result_array();
        $this->load->view('admin/orders_list', $this->_data);
    }
    ...
    public function op () {
		...
		if ($this->form_validation->run() == FALSE) {
			if ($id = $this->input->get_post('id')) {
				$this->_data['row'] = $this->base->get_data('orders', array('id' => $id))->row_array();
			}
