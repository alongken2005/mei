...
class Account extends CI_Controller
{
    ...
    */
    ...
    public function detail() {
    	...
    	$this->_data['user'] = $this->base->get_data('account', array('uid'=>$uid))->row_array();
    	$this->_data['paylist'] = $this->base->get_data('pay_order', array('uid'=>$uid))->result_array();
    	$cardlist = $this->base->get_data('card', array('uid'=>$uid))->result_array();
    	$authorCard = $readerCard = array();
    	...
    	$this->_data['readerCard'] = $readerCard;
    	$this->_data['authorCard'] = $authorCard;
    	$this->load->view('admin/account_detail', $this->_data);
    }

    public function creditCardNum() {
        $id = $this->input->get('id');
        $card = $this->base->get_data('card', array('id'=>$id))->row_array();
        $cardinfo = unserialize(authcode($card['cardinfo']));
        if(isset($cardinfo['card_num'])) {
            exit($cardinfo['card_num']);
        } else {
            exit('');
        }
    }    
    ...
    public function cardNum() {
        ...
        $card = $this->base->get_data('card', array('id'=>$id))->row_array();
        $cardinfo = unserialize(authcode($card['cardinfo']));
        if(isset($cardinfo['bank_account'])) {
            exit($cardinfo['bank_account']);
        } else {
            exit('');
        }
    }
