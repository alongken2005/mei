...
class Index extends MY_Controller {
	...
	public function index() {
		...
		if($this->member && $this->member['uid']) {
			...
			$this->_data['mylist'] = $this->db->query('SELECT * FROM yos_book WHERE uid='.$this->member['uid'].$where.' AND status=1 LIMIT 0, 30')->result_array();

			//Continue Reading
			$this->_data['cread'] = $this->db->query('SELECT b.* FROM yos_history h LEFT JOIN yos_book b ON b.id=h.bid WHERE h.uid='.$this->member['uid'].' AND b.status=1 ORDER BY h.mtime DESC LIMIT 0, 30')->result_array();
		}
		
		//popular on YouShelf
		$this->_data['popular'] = $this->base->get_data('book', array('status'=>1), '*', 0, 30, 'hits DESC')->result_array();

		$this->_data['bestRated'] = $this->base->get_data('book', array('status'=>1), '*', 0, 30, 'score DESC, mtime DESC')->result_array();

		//Top Picks for you
		$this->_data['tops'] = $this->base->get_data('book', array('status'=>1), '*', 0, 30, 'ctime DESC')->result_array();

		$genres = $this->base->get_data('book_genre', array(), 'id, name')->result_array();

		foreach($genres as $v) {
			 $re = $this->base->get_data('book', array('genre'=>$v['id'], 'status'=>1), '*', 0, 30, 'hits DESC')->result_array();
			 if($re) {
