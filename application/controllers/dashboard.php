...
class Dashboard extends MY_Controller {
	...
	public function sales() {		
		...
		$res = $this->db->query("SELECT SUM(payed) price, SUM(words) words, bid, ctime ptime FROM yos_payed_pages WHERE uid=".$this->member['uid']." GROUP BY bid ORDER BY price DESC")->result_array();
		...
		if($bids) {
			$bookRes = $this->db->query('SELECT * FROM yos_book WHERE id IN('.implode(',', $bids).')')->result_array();
			...
			foreach($bookRes as $v) {
				...
				$v['each'] = 0;
				if($v['text_price'] > 0) {
					$v['each'] = intval($pay['price']/$v['text_price']);
				}

				$v['customer_price'] = $v['text_price'];
				$v['proceeds'] = sprintf("%.2f", $rate*$v['text_price']);
				$v['total_customer_price'] = $pay['price'];
				$v['total_proceeds'] = sprintf("%.2f", $rate*$pay['price']);
				$v['ctime'] = $pay['ptime'];
				$v['words'] = $pay['words'];
				$lists[] = $v;
			}
	...
	public function promote() {
		...
		$this->_data['lists'] = $this->base->get_data('book', array('status'=>1, 'uid'=>$this->member['uid']), '*', $limit, $offset, 'id DESC')->result_array();
