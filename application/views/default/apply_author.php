...
<div class="box1000">
	...
	<?php } else if($step == 2) { ?>
		...
		<form method="post" action="<?=site_url('user/apply_author?step=2')?>" class="step2form">
			<img src="<?=base_url('data/images/bank_check.png')?>" class="helpcheck">
			<table cellpadding="0" cellspacing="8" class="book_edit_table">
				<tr>
					<th>Bank Name:</th>
				...
				<tr>
					<th>Bank Account #:</th>
					<td><input type="text" class="input5" name="bank_account"/> <span class="help"></span></td>
				</tr>
				<tr>
					<th>Bank Routing #:</th>
					<td><input type="text" class="input5" name="bank_routing"/> <span class="help2"></span></td>
				</tr>
				<tr>
	...
	<?php } else if($step == 3) {?>
		...
		<form method="post" action="<?=site_url('user/apply_author?step=3')?>">
			...
			<div class="p"><em>3. &nbsp;</em><span>As an author/publisher, you are responsible for any third-party’s claim of rights and damages; you indemnify YouShelf for any such claims and damages related to the content you publish on YouShelf.</span></div>
			...
			<div class="p"><em>8. &nbsp;</em><span>We may need to take down your content due to legal requirement by the government or third-party with legal authorities. We will notify such takedown actions after fact through email and snail mail.</span></div>
			<div class="p"><em>9. &nbsp;</em><span>Until the end of 2016, we pay 100% of the net revenue of all paid content sales to author/publisher publishing via YouShelf. The net revenue is book sales revenue excluding any necessary withholding taxes, banking and payment tool provider fees (e.g., credit card company charges, remittance fees, etc), marketplace platform charges (e.g., Apple app store charges).</span></div>
			<div class="p"><em>10. &nbsp;</em><span>We may adjust the proceed sharing plan anytime and give 60 days written notice ahead of the change through email and snail mail. During the time, you can decide whether to continue to publish the content through us by responding to such notice.</span></div>
			</div>
			<div>
				<input type="hidden" name="agree" value="1"/>
				<input type="submit" class="btn1" value="Agree"/>
				<a href="<?=site_url('user/info')?>" class="btn1">Cancel</a>
				<a href="<?=config_item('authorPdf')?>" target="_blank" class="btn1">View PDF</a>
			</div>			
		</form>	
	<?php } else if($step == 4) { ?>
...
<script type="text/javascript">
	$(function() {
		$('.help').hover(function(){
			$('.helpcheck').css("top", "40px").show();
		}, function() {
			$('.helpcheck').hide();
		});
		$('.help2').hover(function(){
			$('.helpcheck').css("top", "80px").show();
		}, function() {
			$('.helpcheck').hide();
		})		
	});
